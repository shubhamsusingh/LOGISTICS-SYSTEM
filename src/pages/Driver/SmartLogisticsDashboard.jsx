  ...
  if (!GOOGLE_MAPS_API_KEY) {
    return (
      <div className={styles.mapMock}>
        ...
        <div className={styles.mapMockSub}>{STOPS.length - 1} stops plotted along route</div>
        <svg width="88%" height="110" viewBox="0 0 500 110" style={{ marginTop: 6 }}>
          ...
          <polyline points="35,75 130,50 230,40 330,45 445,35"
            fill="none" stroke="#1565c0" strokeWidth="3" strokeDasharray="7,3" />
          {[35, 130, 230, 330, 445].map((x, i) => {
            const ys = [75, 50, 40, 45, 35];
            return (
              <g key={i}>
                <circle cx={x} cy={ys[i]} r={i === 0 ? 11 : 9}
                  fill={i === 0 ? "#0f2d52" : "#e53935"} stroke="#fff" strokeWidth="2" />
                <text x={x} y={ys[i] + 1} textAnchor="middle"
  ...
  return (
    <LoadScript googleMapsApiKey={GOOGLE_MAPS_API_KEY}>
      <div className={styles.mapContainer}>
        <GoogleMap mapContainerStyle={MAP_STYLE} center={MAP_CENTER} zoom={13} options={MAP_OPTIONS}>
          <Polyline
            path={polylinePath}
            options={{
              strokeColor: "#1565c0",
          ...
          {STOPS.map((stop) => (
            <Marker
              ...
              label={{ text: stop.id === 0 ? "W" : String(stop.id), color: "#fff", fontWeight: "bold", fontSize: "12px" }}
              icon={{
          ...
          {selected && (
            <InfoWindow position={{ lat: selected.lat, lng: selected.lng }} onCloseClick={() => setSelected(null)}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#0d2137", padding: "2px 4px" }}>
                {selected.id === 0 ? "🏭 " : `📍 Stop ${selected.id}: `}{selected.label}
              </div>
            </InfoWindow>
          )}
        </GoogleMap>
          ...
          <div className={styles.routeCard}>
            ...
            <div className={styles.stopList}>
              {STOPS.map((stop) => {
                ...
                return (
                  <div key={stop.id} className={`${styles.stopItem} ${done ? styles.done : ""}`}>
                    <div className={`${styles.stopBadge} ${stop.id === 0 ? styles.stopBadgeStart : styles.stopBadgeStop}`}>
                      {stop.id === 0 ? "W" : stop.id}
                    </div>
                    <span className={`${styles.stopLabel} ${done ? styles.done : ""}`}>
                      {stop.id === 0 ? "Start: " : `Stop ${stop.id}: `}{stop.label}
                    </span>
                    {done && <CheckCircleOutlined style={{ color: "#43a047", marginLeft: "auto", fontSize: 13 }} />}
                  </div>
                );
              })}
            </div>
            ...
            <div className={styles.routeLegend}>
              {STOPS.slice(0, 4).map((stop) => (
                <div key={stop.id} className={styles.legendItem}>
                  ...
                  <span>{stop.id === 0 ? "Start: " : `Stop ${stop.id}: `}{stop.label}</span>
                </div>
              ))}
            </div>
          </Card>
        ...
        <div className={styles.actionBar}>
          ...
          <Tooltip title="Mark current stop as delivered">
            <Button type="primary" icon={<CheckCircleOutlined />}
              ...
              onClick={markStop} disabled={stopsCompleted >= totalStops}>
              Update Delivery
            </Button>
          </Tooltip>
        ...
        <div className={styles.bottomGrid}>
          ...
          <Card className={styles.summaryCard} bodyStyle={{ padding: "20px" }}>
            ...
            <div className={styles.progressSection}>
              ...
              <Progress percent={stopsPct} showInfo={false}
                strokeColor={{ from: "#1565c0", to: "#42a5f5" }}
                trailColor="#e8edf3" strokeWidth={8} />
            </div>
            ...
            <div className={styles.progressSection}>
              ...
              <Progress percent={loadPct} showInfo={false}
          ...
          <Card className={styles.emergencyCard} bodyStyle={{ padding: "20px" }}>
            ...
            <div className={styles.emergencyMeta}>
              <strong>Vehicle:</strong> {ROUTE_INFO.vehicle}<br />
